...
#include "scheduler_by_pq_by_sortl.h"
...
int ActionFunc(void *data)
{
	UNUSED(data);
	
	printf("Action Function is at process\n");
	return 1;
}
...
static void CreateTest()
{
	...
	, SchedIsEmpty(sched), 1, __LINE__); 
	
	/*---------------- Test #2 ----------------*/
	SchedAddTask(sched, &ActionFunc, time_to_run, interval_in_sec, param
								,&CleanFunc , clean_params);
	fail_message = "Test 2 failed - "BOLD_RED"SchedAddTask"RESET;
	PrintTestResult(fail_message
	, SchedSize(sched), 1, __LINE__);
	...
	/*---------------- Test #4 ----------------*/
	uid = SchedAddTask(sched, &ActionFunc, time_to_run, interval_in_sec, param
								,&CleanFunc , clean_params);
	fail_message = "Test 4 failed - "BOLD_RED"SchedRemove"RESET;
	PrintTestResult(fail_message
	, SchedRemove(sched, uid), 0, __LINE__);
	
	/*---------------- Test #5 ----------------*/
	uid = SchedAddTask(sched, &ActionFunc, time_to_run, interval_in_sec, param, &CleanFunc , clean_params);
	
	task_rem.sch = sched;
	task_rem.u_id = uid;
	
	SchedAddTask(sched, &RemoveFunc, time_to_run + 2, interval_in_sec, (void *)&task_rem, &CleanFunc , clean_params);
	
	SchedAddTask(sched, &StopFunc, time_to_run + 5, interval_in_sec, param
								,&CleanFunc , clean_params);	
								
	fail_message = "Test 5 failed - "BOLD_RED"SchedRun"RESET;
	PrintTestResult(fail_message
	, SchedRun(sched), 1, __LINE__);
